...
if __name__ == "__main__":
    connection = sqlite3.connect("/home/musasina/projects/ZiraatBankFraudAI/gemini_dataset/collected_data.db")
    cursor = connection.cursor()
    count = 0
    extra_prompt = ""
    names = pd.read_csv("/home/musasina/projects/ZiraatBankFraudAI/datasets/names.csv").convert_dtypes(convert_string=True)["names"].to_list()
    cities = pd.read_csv("/home/musasina/projects/ZiraatBankFraudAI/datasets/cities.csv").convert_dtypes(convert_string=True)["names"].to_list()
    companies = pd.read_csv("/home/musasina/projects/ZiraatBankFraudAI/datasets/companies.csv").convert_dtypes(convert_string=True)["names"].to_list()
    surnames = pd.read_csv("/home/musasina/projects/ZiraatBankFraudAI/datasets/surnames.csv").convert_dtypes(convert_string=True)["names"].to_list()
    jobs = pd.read_csv("/home/musasina/projects/ZiraatBankFraudAI/datasets/job_list.csv").convert_dtypes(convert_string=True)["names"].to_list()
    jobs = list(set(jobs))
    while True:
        try:
            ...
            job_year = abs(np.random.randint(0,76-age))
