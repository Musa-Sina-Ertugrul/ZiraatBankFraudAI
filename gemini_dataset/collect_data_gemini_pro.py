...
prompt = "Bana fraud veya not fraud tespiti hikayesi yazman lazım: \
    ...
    while True:
        try:
            ...
            extra_prompt = f"""17. Kişi ismi {choiced_name} olmalı 
                                ...
                                33. Hikayeyi hikayenin kahramanı veya olayı yaşayan kişi anlatmalı
                                """
            ...
            try:
                ...
                cursor.execute(f"INSERT INTO data(text, label) VALUES('{clear_punc(response_as_dict[0]['text'])}', '{clear_punc(response_as_dict[0]['label'])}');")
            connection.commit()
            count += 1
